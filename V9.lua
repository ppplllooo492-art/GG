-- [[ 🌟 CAMP SCRIPT HUB - MM2 GOD MODE V9 (ULTIMATE FLING EXPLODE) 🌟 ]]
-- รันบน Delta 100% | ระบบเตะวาร์ปสิงร่างชนแตกหลุดแมป 100% | กระสุนล็อกวาร์ปปักลำตัว | ESP Real-time
...
ScreenGui.Name = "CampScript_Premium_Hub_V9"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")

----------------------------------------------------------------
-- 1. ฟังก์ชันแกนหลัก: เช็กบทบาทและค้นหาวัตถุ (Core Scan Engine)
----------------------------------------------------------------
...
----------------------------------------------------------------
-- 2. แผงควบคุมระบบปุ่มเลื่อนและคำสั่งเตะ (Control Panel UI)
----------------------------------------------------------------
...
KickLabel.Text = "God Fling Explode (100%)"
...
-- 4. ฟังก์ชันการทำงานของปุ่มไอคอน (Kill All & Unfair Silent Aim)
    ...
    while task.wait(0.1) do
        if autoGrabEnabled then
            ...
            if gunDrop and root and not char:FindFirstChild("Gun") and not LocalPlayer.Backpack:FindFirstChild("Gun") then
                ...
                repeat
                    if not gunDrop or not gunDrop:FindFirstChild("Handle") then break end
...
-- 7. [[ 😈 NEW FLING ENGINE V10 - แยกฝั่งชนแตก 100% ตัวเราปลอดภัยไม่พัง ]]

local function isolatedFlingExplode(targetPlayer)
    ...
    -- [🛡️ PROTECTION] ล็อกฟิสิกส์ให้ตัวเราปลอดภัย (ไม่ติดสถานะ PlatformStand และไม่ยืดร่างตามเป้าหมาย)
    ...
    -- [🔥 FORCE INJECTION] บังคับสร้างแรงหมุนความเร็วสูงระดับโมเลกุลที่ตัวเหยื่อโดยตรง (ยัดค่าส่งบั๊กใส่ตัวละครเป้าหมาย)
    local enemyThrust = Instance.new("BodyThrust")
    enemyThrust.Force = Vector3.new(999999, 999999, 999999)
    enemyThrust.Location = enemyRoot.Position
    enemyThrust.Parent = enemyRoot -- ยัดลงตัวเหยื่อ!

    local angularV = Instance.new("AngularVelocity")
    angularV.MaxTorque = math.huge
    angularV.AngularVelocity = Vector3.new(0, 999999, 0)
    angularV.Attachment0 = enemyRoot:FindFirstChildOfClass("Attachment") or Instance.new("Attachment", enemyRoot)
    angularV.Parent = enemyRoot -- ยัดลงตัวเหยื่อ!

    -- ดึงตัวเราวาร์ปไปประกบติดเพื่อส่งสัญญาณเน็ตเวิร์ก (Network Ownership Glitch)
    local startTime = tick()
    repeat
        if not enemyRoot or not enemyRoot.Parent then break end
        -- วาร์ปเราไปชนตรงๆ ในพิกัดเยื้องเล็กน้อยเพื่อส่งแรงกระแทกจากเครื่องเรา บังคับให้เซิร์ฟเวอร์เคลียร์ฟิสิกส์เหยื่อ
        myRoot.CFrame = enemyRoot.CFrame * CFrame.new(math.random(-1, 1)/100, 0, math.random(-1, 1)/100)
        myRoot.Velocity = Vector3.new(0, 200, 0)
        task.wait()
    until not targetPlayer or not targetPlayer.Character or not enemyRoot.Parent or enemyHum.Health <= 0 or (tick() - startTime > 1.3)

    -- เคลียร์ออบเจ็กต์ฟิสิกส์แปลกปลอมออกจากห้อง และดึงเรากลับจุดเดิมอย่างปลอดภัย
    noclipLoop:Disconnect()
    if enemyThrust.Parent then enemyThrust:Destroy() end
    if angularV.Parent then angularV:Destroy() end

    if myRoot and myRoot.Parent then
        myRoot.Velocity = Vector3.new(0, 0, 0)
        myRoot.RotVelocity = Vector3.new(0, 0, 0)

        -- คืนค่าระบบชนให้ตัวเรา
        for _, part in ipairs(myChar:GetChildren()) do
            if part:IsA("BasePart") then part.CanCollide = true end
        end

        task.wait(0.05)
        myRoot.CFrame = originalCFrame -- ดึงตัวเรากลับมาตำแหน่งเดิมก่อนรันคำสั่งวาร์ปทันที
    end
end
...
KickMudBtn.MouseButton1Click:Connect(function()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and getRole(p) == "MURDERER" then isolatedFlingExplode(p) end
    end
end)

KickSherBtn.MouseButton1Click:Connect(function()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and getRole(p) == "SHERIFF" then isolatedFlingExplode(p) end
    end
end)

KickInnoBtn.MouseButton1Click:Connect(function()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and getRole(p) == "INNOCENT" then isolatedFlingExplode(p) end
    end
end)
